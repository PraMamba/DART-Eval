...
class VariantSingleTokenLikelihoodEvaluator(LikelihoodEvaluator):
	...
	def evaluate(self, dataset, output_file, progress_bar=True):
		...
		dataset_chunks = torch.utils.data.random_split(
			dataset,
			[len(dataset) // num_gpus + int(i < len(dataset) % num_gpus) for i in range(num_gpus)]
		)

		ctx = mp.get_context("spawn")
		processes = []

		# ✅ 用 Manager().list 替代 Queue（初始化为 None）
		manager = mp.Manager()
		...
		for rank in range(num_gpus):
			p = ctx.Process(
				...
				args=(dataset_chunks[rank], rank, f"{base}_part{rank}{ext}", shared_results, progress_bar)
			)
			p.start()
			processes.append(p)
		...
		for rank in range(num_gpus):
			...
			if result is None:
				...
				allele1_scores, allele2_scores = [], []
			else:
				allele1_scores, allele2_scores = result

			all_allele1.extend(allele1_scores)
			all_allele2.extend(allele2_scores)

		data = {
			"allele1_scores": all_allele1,
			"allele2_scores": all_allele2
		}
	...
	def _evaluate_worker(self, dataset_split, gpu_id, output_file, shared_results, progress_bar):
		try:
			...
			with open(output_file, "w") as f:
				pbar = tqdm(dataloader, disable=(not progress_bar), ncols=120, position=gpu_id, desc=f"[GPU {gpu_id}]", leave=True)
				for batch_idx, (allele1, allele2) in enumerate(pbar):
					try:
						tokens_allele1, starts1, ends1, attn1 = self.tokenize(allele1)
						tokens_allele2, starts2, ends2, attn2 = self.tokenize(allele2)

						diffs = tokens_allele1 != tokens_allele2
						tokens_masked = tokens_allele1.clone()
						tokens_masked[diffs] = self.mask_token

						lls1 = self.score(tokens_masked, tokens_allele1, starts1, ends1, attn1, allele1)
						lls2 = self.score(tokens_masked, tokens_allele2, starts2, ends2, attn2, allele2)

						for l1, l2 in zip(lls1.flatten(), lls2.flatten()):
							allele1_likelihoods.append(l1)
							allele2_likelihoods.append(l2)
							f.write(f"{l1}\t{l2}\n")
						f.flush()
			...
			# ✅ 替代 queue.put()：写入 shared_results
			shared_results[gpu_id] = (allele1_likelihoods, allele2_likelihoods)
	...
	def score(self, tokens_in, tokens_out, starts, ends, attention_mask, seq):
		...
		clip_mask = torch.tensor(
			[[(i >= s) and (i < e) for i in range(lls.shape[1])] for s, e in zip(starts, ends)],
			dtype=torch.float
		).to(device=self.device)

		return (lls * clip_mask).sum(1).numpy(force=True)
